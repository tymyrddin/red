...
Scenario: The [OpenHands International](entity.md) network is dual-stack (has IPv4 and IPv6 enabled). However, their network monitoring and security policies are primarily focused on IPv4. Your goal is to exploit IPv6 auto-configuration to redirect and intercept victim traffic. The attack surfaces are modelled in [Rootways: IP](../../in/network/roots/ip/index.rst). Procedures for the router advertisement spoofing phase are in [Tradecraft: layer 2 attacks](../../in/network/runbooks/layer2.md).
...
4.  Identify the Legitimate Router:
    ...
    *   Finding: note the source IPv6 address of the legitimate router. It matches `LEGITIMATE_RA`.
...
3.  Verify the attack is working:
    ...
    *   Finding: the victim now has *two* default routes: one to the legitimate router and one to the attacker machine. IPv6 will often prefer the most recently received RA.
...
3.  Trigger and Verify DNS Redirection:
    ...
    *   Finding: the `nslookup` shows that the DNS query was answered by the attacker's IPv6 address. The result IP is also the attacker IP.
