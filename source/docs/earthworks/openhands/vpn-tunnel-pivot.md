...
Scenario: [OpenHands International](entity.md) uses an IPSec VPN tunnel to connect its HQ to a remote branch office. The tunnel is established, but a weak pre-shared key (PSK) and permissive firewall rules within the tunnel create an opportunity for pivot and lateral movement. The attack surfaces are modelled in [Rootways: IPsec](../../in/network/roots/ipsec/index.rst). Procedures for discovery and pivot are in [Tradecraft: network reconnaissance](../../in/network/runbooks/network-recon.md) and [Tradecraft: lateral movement](../../in/network/runbooks/lateral-movement.md).
...
1.  Exploit the Service:
    ...
    *   Metasploit Commands:
        ...
        msf6 exploit(...) > run
        ```
    *   Success: a `meterpreter` shell opens.
