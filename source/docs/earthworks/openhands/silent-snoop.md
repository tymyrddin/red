...
Scenario: The [OpenHands International](entity.md) office LAN uses a flat network structure with minimal segmentation. Your goal is to establish a persistent MitM position on this local network segment to intercept unencrypted traffic and redirect users to a malicious credential-harvesting portal. The attack surfaces are modelled in [Rootways: IP](../../in/network/roots/ip/index.rst). Procedures are in [Tradecraft: layer 2 attacks](../../in/network/runbooks/layer2.md) and [Tradecraft: name resolution](../../in/network/runbooks/name-resolution.md).
...
2.  Discover Your Network Configuration:
    *   The machine will have received an IP address via DHCP. Find the IP, subnet mask, and the default gateway.
...
3.  Verify the attack is working:
    *   On the attacker machine, check the ARP table. It correctly maps the IPs to their real MAC addresses.
    *   On the victim machine (ask the instructor for access or simulate it), check its ARP table. The `GATEWAY_IP` is now mapped to the attacker machine's MAC address.
    *   Command on Victim (simulated): `arp -a`
    *   Finding: What MAC address does the victim have for the gateway? It is the attacker's MAC address.
...
3.  Force Clients to Reconnect:
    ...
    *   Verification: after the victim renews, the new gateway and DNS server entries are the attacker IP.
