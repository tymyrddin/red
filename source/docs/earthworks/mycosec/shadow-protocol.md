...
Scenario: The [MycoSec](entity.md) "Singularity" lab is a cutting-edge, IPv6-only network segment used for protocol research. It has no IPv4 connectivity. Your goal is to exploit the inherent trust in IPv6 autoconfiguration and services to own the network. The attack surfaces are modelled in [Rootways: IP](../../in/network/roots/ip/index.rst). Procedures for the router advertisement phase are in [Tradecraft: layer 2 attacks](../../in/network/runbooks/layer2.md); the covert channel phase draws on [Tradecraft: C2 tunnelling](../../in/network/runbooks/c2-tunneling.md).
...
2.  Confirm IPv6-Only Environment:
    ...
    *   Expected Result: no IPv4 addresses or routes appear.
...
2.  Research and Execute the Exploit:
    ...
    *   Success: the script injects a command that downloads and executes a reverse shell payload back to the attacker VM.

3.  Establish a Reverse Shell:
    *   On your attacker VM, start a netcat listener on an IPv6 socket.
    *   Command: `sudo nc -6 -l -v -p 4444`
    *   Verification: a reverse shell connection arrives from the router with root privileges.
...
4.  Trigger a Network Reconfiguration:
    ...
    *   Verification: the target's new IPv6 routing table and DNS resolver configuration list the attacker VM's IP for both.
