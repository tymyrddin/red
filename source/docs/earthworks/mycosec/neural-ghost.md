...
Scenario: [MycoSec](entity.md)'s "Cortex" lab network uses standard security tools (firewalls, IDS, basic SIEM). Your goal is to use AI to bypass these defences, find novel attack paths, and maintain stealthy persistence. The reconnaissance phase draws on [Tradecraft: network reconnaissance](../../in/network/runbooks/network-recon.md).
