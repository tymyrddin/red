...
Scenario: The [MycoSec](entity.md) internal lab network is segmented into trust zones, including a development virtual local area network and a secured management virtual local area network. Development servers possess trusted relationships with management systems for deployment tasks. The goal is to pivot from a low-value development server to a high-value management server. Internal routing trust surfaces are modelled in [Patches](../../in/network/patches/index.rst). Procedures for the C2 establishment and lateral movement phases are in [Tradecraft: C2 tunnelling](../../in/network/runbooks/c2-tunneling.md) and [Tradecraft: lateral movement](../../in/network/runbooks/lateral-movement.md).
