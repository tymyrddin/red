...
Exploiting XXE to retrieve data by repurposing a local DTD is the case here. Disabling external entities and DTD processing in the parser removes it. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
