...
Blind XXE with out-of-band interaction via XML parameter entities is the variant in play. Disabling external entities and DTD processing in the parser removes it. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
