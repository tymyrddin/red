...
Exploiting blind XXE to retrieve data via error messages is the case here. Disabling external entities and DTD processing in the parser removes it. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
