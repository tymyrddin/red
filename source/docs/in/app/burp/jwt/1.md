...
JWT authentication bypass via unverified signature is what this page works through. Pinning the algorithm, verifying signatures, and rejecting algorithm confusion are the counters. Seen from the other side, this sits in the blue notes on [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
