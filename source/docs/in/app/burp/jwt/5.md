...
JWT authentication bypass via jku header injection is the case here. Pinning the algorithm, verifying signatures, and rejecting algorithm confusion are the counters. Seen from the other side, this sits in the blue notes on [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
