...
Exploiting HTTP request smuggling to deliver reflected XSS is what this page works through. The fix is a front-end and back-end that agree on request boundaries, ideally HTTP/2 end to end. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
