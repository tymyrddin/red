...
Bypassing access controls via HTTP/2 request tunnelling is the variant in play. The fix is a front-end and back-end that agree on request boundaries, ideally HTTP/2 end to end. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
