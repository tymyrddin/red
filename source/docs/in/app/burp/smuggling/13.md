...
[This lab](https://portswigger.net/web-security/request-smuggling/advanced/lab-request-smuggling-h2-request-smuggling-via-crlf-injection) is vulnerable to request smuggling because the front-end server downgrades HTTP/2 requests and fails to adequately sanitise incoming headers.
...
HTTP/2 request smuggling via CRLF injection is what this page works through. The fix is a front-end and back-end that agree on request boundaries, ideally HTTP/2 end to end. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
