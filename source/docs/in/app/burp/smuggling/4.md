...
HTTP request smuggling, confirming a CL.TE vulnerability via differential responses is what this page works through. The fix is a front-end and back-end that agree on request boundaries, ideally HTTP/2 end to end. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
