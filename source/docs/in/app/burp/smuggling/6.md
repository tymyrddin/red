...
Exploiting HTTP request smuggling to bypass front-end security controls, CL.TE vulnerability is the variant in play. The fix is a front-end and back-end that agree on request boundaries, ideally HTTP/2 end to end. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
