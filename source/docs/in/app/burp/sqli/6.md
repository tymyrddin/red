...
SQL injection UNION attack, retrieving multiple values in a single column is the case here. Parameterised queries and least-privilege database accounts are the durable answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
