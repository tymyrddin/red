...
SQL injection attack, listing the database contents on Oracle is the case here. Parameterised queries and least-privilege database accounts are the durable answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
