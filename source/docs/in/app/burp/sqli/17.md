...
SQL injection with filter bypass via XML encoding is the variant in play. Parameterised queries and least-privilege database accounts are the durable answer. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
