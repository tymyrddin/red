...
SQL injection attack, querying the database type and version on Oracle is the variant in play. Parameterised queries and least-privilege database accounts are the durable answer. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
