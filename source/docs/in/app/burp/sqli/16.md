...
Blind SQL injection with out-of-band data exfiltration is the case here. Parameterised queries and least-privilege database accounts are the durable answer. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
