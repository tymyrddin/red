...
SQL injection UNION attack, determining the number of columns returned by the query is the case here. Parameterised queries and least-privilege database accounts are the durable answer. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
