...
SQL injection UNION attack, finding a column containing text is the variant in play. Parameterised queries and least-privilege database accounts are the durable answer. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
