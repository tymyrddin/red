...
File path traversal, traversal sequences blocked with absolute path bypass is the variant in play. Canonicalising paths and confining access to an allow-listed root close it. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
