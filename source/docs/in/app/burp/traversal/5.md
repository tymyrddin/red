...
File path traversal, validation of start of path is the variant in play. Canonicalising paths and confining access to an allow-listed root close it. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
