...
Using PHAR deserialisation to deploy a custom gadget chain is the variant in play. Avoiding native deserialisation of untrusted data, or constraining it with type allow-lists, is the fix. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
