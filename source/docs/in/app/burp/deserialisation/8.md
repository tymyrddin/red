...
Developing a custom gadget chain for Java deserialisation is what this page works through. Avoiding native deserialisation of untrusted data, or constraining it with type allow-lists, is the fix. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
