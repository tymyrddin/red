...
Exploiting PHP deserialisation with a pre-built gadget chain is the case here. Avoiding native deserialisation of untrusted data, or constraining it with type allow-lists, is the fix. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
