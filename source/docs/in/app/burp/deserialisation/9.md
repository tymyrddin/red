...
Developing a custom gadget chain for PHP deserialisation is the case here. Avoiding native deserialisation of untrusted data, or constraining it with type allow-lists, is the fix. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
