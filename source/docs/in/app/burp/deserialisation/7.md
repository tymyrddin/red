...
Exploiting Ruby deserialisation using a documented gadget chain is the variant in play. Avoiding native deserialisation of untrusted data, or constraining it with type allow-lists, is the fix. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
