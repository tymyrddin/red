...
Server-side template injection with information disclosure via user-supplied objects is the case here. Keeping user input out of templates and sandboxing the engine close it. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
