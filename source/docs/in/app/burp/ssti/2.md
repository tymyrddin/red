...
Basic server-side template injection (code context) is the case here. Keeping user input out of templates and sandboxing the engine close it. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
