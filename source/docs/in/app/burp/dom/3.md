...
DOM XSS using web messages and JSON.parse is the case here. Avoiding dangerous sinks and using safe DOM APIs are the answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
