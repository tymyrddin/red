...
Clobbering DOM attributes to bypass HTML filters is the variant in play. Avoiding dangerous sinks and using safe DOM APIs are the answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
