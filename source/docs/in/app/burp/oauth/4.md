...
Stealing OAuth access tokens via an open redirect is the variant in play. Validating redirect URIs, state, and tokens server-side are the counters. The defender's view is in the blue notes on [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
