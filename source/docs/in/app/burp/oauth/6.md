...
Stealing OAuth access tokens via a proxy page is the case here. Validating redirect URIs, state, and tokens server-side are the counters. The defensive counterpart is in the blue notes on [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
