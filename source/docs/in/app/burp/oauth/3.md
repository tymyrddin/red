...
OAuth account hijacking via redirect_uri is the case here. Validating redirect URIs, state, and tokens server-side are the counters. Defenders' notes on this are under [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
