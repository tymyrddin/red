...
Authentication bypass via OAuth implicit flow is the variant in play. Validating redirect URIs, state, and tokens server-side are the counters. Seen from the other side, this sits in the blue notes on [the surface designed to be accessible](https://blue.tymyrddin.dev/docs/counter/api/).
