...
CSRF where token validation depends on token being present is what this page works through. Anti-CSRF tokens and SameSite cookies are the standard defence. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
