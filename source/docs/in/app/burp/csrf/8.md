...
SameSite Strict bypass via client-side redirect is the variant in play. Anti-CSRF tokens and SameSite cookies are the standard defence. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
