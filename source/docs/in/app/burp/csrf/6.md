...
CSRF where token is duplicated in cookie is what this page works through. Anti-CSRF tokens and SameSite cookies are the standard defence. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
