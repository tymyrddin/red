...
CSRF where Referer validation depends on header being present is what this page works through. Anti-CSRF tokens and SameSite cookies are the standard defence. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
