...
CSRF where token validation depends on request method is the variant in play. Anti-CSRF tokens and SameSite cookies are the standard defence. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
