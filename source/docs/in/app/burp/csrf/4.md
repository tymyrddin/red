...
CSRF where token is not tied to user session is the case here. Anti-CSRF tokens and SameSite cookies are the standard defence. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
