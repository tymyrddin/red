...
Privilege escalation via server-side prototype pollution is the case here. Strict parameter parsing, and not merging untrusted input into object prototypes, close it. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
