...
Weak isolation on dual-use endpoint is what this page works through. Enforcing the rules server-side and validating each state transition are the counters. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
