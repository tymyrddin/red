...
High-level logic vulnerability is the case here. Enforcing the rules server-side and validating each state transition are the counters. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
