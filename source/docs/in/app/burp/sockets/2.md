...
Manipulating the WebSocket handshake to exploit vulnerabilities is the case here. Authenticating the handshake and validating every message are the levers. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
