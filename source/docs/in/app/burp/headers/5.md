...
SSRF via flawed request parsing is the case here. Stripping revealing headers and setting the right security headers are the fix. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
