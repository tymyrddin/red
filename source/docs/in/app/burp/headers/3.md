...
Web cache poisoning via ambiguous requests is the variant in play. Stripping revealing headers and setting the right security headers are the fix. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
