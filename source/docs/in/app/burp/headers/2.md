...
Host header authentication bypass is the case here. Stripping revealing headers and setting the right security headers are the fix. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
