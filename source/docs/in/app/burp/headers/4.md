...
Routing-based SSRF is what this page works through. Stripping revealing headers and setting the right security headers are the fix. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
