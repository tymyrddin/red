...
CORS vulnerability with trusted insecure protocols is the variant in play. An allow-list of trusted origins, and never reflecting an arbitrary Origin with credentials, are the counters. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
