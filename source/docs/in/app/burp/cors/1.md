...
CORS vulnerability with basic origin reflection is what this page works through. An allow-list of trusted origins, and never reflecting an arbitrary Origin with credentials, are the counters. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
