...
Information disclosure on debug page is the variant in play. Authorising every object access against the caller, not the supplied identifier, is the answer. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
