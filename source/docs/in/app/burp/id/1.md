...
Information disclosure in error messages is the case here. Authorising every object access against the caller, not the supplied identifier, is the answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
