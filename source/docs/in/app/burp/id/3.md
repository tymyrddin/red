...
Source code disclosure via backup files is what this page works through. Authorising every object access against the caller, not the supplied identifier, is the answer. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
