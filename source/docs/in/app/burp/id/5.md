...
Information disclosure in version control history is the variant in play. Authorising every object access against the caller, not the supplied identifier, is the answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
