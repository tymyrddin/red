...
Username enumeration via different responses is the variant in play. Rate limiting, MFA, and uniform failure messages are the levers. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
