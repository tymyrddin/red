...
2FA bypass using a brute-force attack is the case here. Rate limiting, MFA, and uniform failure messages are the levers. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
