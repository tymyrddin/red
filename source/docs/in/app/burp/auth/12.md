...
Password brute-force via password change is the variant in play. Rate limiting, MFA, and uniform failure messages are the levers. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
