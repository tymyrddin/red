...
Reflected XSS in a JavaScript URL with some characters blocked is the variant in play. Context-aware output encoding and a strong Content-Security-Policy close the class, whatever the sink. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
