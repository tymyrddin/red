...
Reflected XSS protected by CSP, with CSP bypass is the case here. Context-aware output encoding and a strong Content-Security-Policy close the class, whatever the sink. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
