...
Reflected XSS into HTML context with nothing encoded is the variant in play. Context-aware output encoding and a strong Content-Security-Policy close the class, whatever the sink. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
