...
Reflected XSS with event handlers and href attributes blocked is the case here. Context-aware output encoding and a strong Content-Security-Policy close the class, whatever the sink. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
