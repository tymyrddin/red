...
Blind OS command injection with out-of-band interaction is the variant in play. Avoiding the shell and passing arguments as data, never building command strings, is the fix. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
