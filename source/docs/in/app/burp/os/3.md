...
Blind OS command injection with output redirection is the case here. Avoiding the shell and passing arguments as data, never building command strings, is the fix. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
