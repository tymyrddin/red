...
Blind OS command injection with time delays is what this page works through. Avoiding the shell and passing arguments as data, never building command strings, is the fix. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
