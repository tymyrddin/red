...
Parameter cloaking is the case here. Keying the cache on every input that changes the response, and not caching authenticated content, are the counters. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
