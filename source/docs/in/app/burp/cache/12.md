...
Cache key injection is what this page works through. Keying the cache on every input that changes the response, and not caching authenticated content, are the counters. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
