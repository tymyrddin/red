...
Web cache poisoning with an unkeyed cookie is the variant in play. Keying the cache on every input that changes the response, and not caching authenticated content, are the counters. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
