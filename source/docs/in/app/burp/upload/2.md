...
Web shell upload via Content-Type restriction bypass is the variant in play. Server-side type validation, storage outside the webroot, and never executing uploads are the levers. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
