...
Web shell upload via path traversal is what this page works through. Server-side type validation, storage outside the webroot, and never executing uploads are the levers. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
