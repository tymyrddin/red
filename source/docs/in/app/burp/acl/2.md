...
Unprotected admin functionality with unpredictable URL is what this page works through. Server-side authorisation on every request, defaulting to deny, is the answer. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
