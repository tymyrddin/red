...
User role controlled by request parameter is the case here. Server-side authorisation on every request, defaulting to deny, is the answer. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
