...
User ID controlled by request parameter, with unpredictable user IDs is the case here. Server-side authorisation on every request, defaulting to deny, is the answer. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
