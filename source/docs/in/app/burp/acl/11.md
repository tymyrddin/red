...
[This lab](https://portswigger.net/web-security/access-control/lab-method-based-access-control-can-be-circumvented) implements access controls based partly on the HTTP method of requests. You can familiarise yourself with the admin panel by logging in using the credentials `administrator:admin`. 
...
Method-based access control can be circumvented is what this page works through. Server-side authorisation on every request, defaulting to deny, is the answer. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
