...
Referer-based access control is the variant in play. Server-side authorisation on every request, defaulting to deny, is the answer. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
