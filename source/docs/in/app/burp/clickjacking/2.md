...
Clickjacking with form input data prefilled from a URL parameter is the variant in play. Frame-ancestors in CSP, or X-Frame-Options, are the standard defence. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
