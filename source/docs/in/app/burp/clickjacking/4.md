...
Exploiting clickjacking vulnerability to trigger DOM-based XSS is the case here. Frame-ancestors in CSP, or X-Frame-Options, are the standard defence. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
