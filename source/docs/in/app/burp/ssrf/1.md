...
Basic SSRF against the local server is the variant in play. Allow-listing outbound destinations and blocking the metadata endpoint are the counters. The defender's view is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
