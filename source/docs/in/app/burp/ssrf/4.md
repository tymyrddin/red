...
SSRF with filter bypass via open redirection vulnerability is the variant in play. Allow-listing outbound destinations and blocking the metadata endpoint are the counters. Defenders' notes on this are under [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
