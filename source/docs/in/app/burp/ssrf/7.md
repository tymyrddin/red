...
Blind SSRF with Shellshock exploitation is the variant in play. Allow-listing outbound destinations and blocking the metadata endpoint are the counters. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
