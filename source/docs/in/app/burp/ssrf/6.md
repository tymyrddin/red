...
SSRF with whitelist-based input filter is the case here. Allow-listing outbound destinations and blocking the metadata endpoint are the counters. Seen from the other side, this sits in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
