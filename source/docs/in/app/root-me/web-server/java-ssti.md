...
Java: Server-side Template Injection (SSTI) is the variant in play. Server-side validation and least privilege are what these reduce to. The defensive counterpart is in the blue notes on [the application layer as a target](https://blue.tymyrddin.dev/docs/counter/app/).
